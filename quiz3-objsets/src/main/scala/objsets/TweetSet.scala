...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new NoSuchElementException("Empty set has no mostRetweeted")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val filteredSubs = right.filterAcc(p, filteredLeft)
    if (p(this.elem)) filteredSubs.incl(elem) else filteredSubs
  }
